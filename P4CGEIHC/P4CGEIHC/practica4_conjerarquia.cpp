...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		// ================================
		// EJERCICIO DE PRACTICA - TERCER BRAZO
		// ================================
		model = modelaux;

		// tercer brazo
		// se extiende hacia arriba-derecha para acercarse a la referencia
		...
		// CANASTA
		// ================================
		glm::mat4 canastaBase = modelaux;

		// mover toda la canasta para que la articulación quede cerca de una esquina
		canastaBase = glm::translate(canastaBase, glm::vec3(0.0f, 0.0f, 1.75f));

		color = glm::vec3(0.8f, 0.8f, 0.8f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));

		// base
		model = glm::translate(canastaBase, glm::vec3(-1.75f, 0.0f, -1.75f));
		model = glm::scale(model, glm::vec3(3.5f, 0.5f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();

		// pared izquierda
		model = glm::translate(canastaBase, glm::vec3(-3.35f, 1.0f, -1.75f));
		model = glm::scale(model, glm::vec3(0.25f, 2.0f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();

		// pared derecha
		model = glm::translate(canastaBase, glm::vec3(-0.15f, 1.0f, -1.75f));
		model = glm::scale(model, glm::vec3(0.25f, 2.0f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();

		// pared trasera
		model = glm::translate(canastaBase, glm::vec3(-1.75f, 1.0f, -3.35f));
		model = glm::scale(model, glm::vec3(3.5f, 2.0f, 0.25f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();

		// barandal frontal inferior
		model = glm::translate(canastaBase, glm::vec3(-1.75f, 0.35f, -0.15f));
		model = glm::scale(model, glm::vec3(3.5f, 0.35f, 0.25f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();

		// barandal frontal superior
		model = glm::translate(canastaBase, glm::vec3(-1.75f, 1.85f, -0.15f));
		model = glm::scale(model, glm::vec3(3.5f, 0.25f, 0.25f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		meshList[0]->RenderMesh();
